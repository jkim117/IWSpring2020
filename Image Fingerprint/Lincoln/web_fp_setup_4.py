...
def parse_tcp(ip_packet, ts):
    ...
    if client_port in entry["sessions"]:
        ...
        if seq_num < entry['sessions'][client_port][1]:
            ...
            seq_0 = entry['sessions'][client_port][0]
            if seq_num - seq_0 < -100000000: # We probably have a situation of Sequence Number Wraparound
                netassayTable[key]['sessions'][client_port][0] = -(4294967295 - seq_0)
                netassayTable[key]['sessions'][client_port][1] = seq_num
    ...
    with open(argv[1].split('.')[0] + '.csv', 'w') as csvfile:
        ...
        for key in netassayTable:
            ...
            for port in sessions:
                ...
                if sessions[port][0] < 0:
                    seqdiff = -sessions[port][0] + sessions[port][1]
                else:
