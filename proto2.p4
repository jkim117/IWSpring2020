...
parser TopParser(packet_in pkt,
    ...
    state parse_cname {
        pkt.advance((bit<32>)p.dns_answer.rd_length);
    ...
    apply {
        if(user_metadata.parsed_answer == 1) {
            ...
            if (user_metadata.matched_domain == 1) {
                ...
                if (user_metadata.temp_timestamp == 0 || user_metadata.temp_timestamp + TIMEOUT < standard_metadata.ingress_global_timestamp || (user_metadata.temp_cip == headers.ipv4.dst && user_metadata.temp_sip == headers.dns_ip.rdata)) {
